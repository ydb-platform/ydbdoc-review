...
| `YDBDOC_TRANSLATE_MAX_OUTPUT_TOKENS` | Потолок выходных токенов **перевода** (по умолчанию **96000**). Увеличьте, если длинные EN-статьи обрываются посреди блока кода. |
...
- Лимит **выходных** токенов перевода задаётся **`YDBDOC_TRANSLATE_MAX_OUTPUT_TOKENS`** (по умолчанию **96000**). Слишком маленькое значение даёт обрыв статьи посреди кода; при подозрении на обрыв делается **второй** вызов с удвоенным лимитом (до потолка). Очень длинные статьи при необходимости поднимите лимит или разбейте PR.
