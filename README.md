...
Имя **секрета в GitHub** не может начинаться с префикса **`GITHUB_`** (ограничение платформы). Поэтому PAT хранится, например, как **`YDBDOC_PUSH_PAT`**, а в `env` action передаётся в переменную окружения **`GITHUB_PUSH_TOKEN`**:
    ...
    steps:
      ...
      - name: Documentation translation review
        ...
        env:
          ...
          GITHUB_PUSH_TOKEN: ${{ secrets.YDBDOC_PUSH_PAT }}  # имя секрета без префикса GITHUB_
