...
Официальный перечень в интерфейсе: [Yandex AI Studio — Model gallery](https://aistudio.yandex.ru/model-gallery). Для cross-check (критик) по умолчанию в TOML образа: **`qwen3.6-35b-a3b`** (`[models].translation_verify`). Переопределение: **`YDBDOC_MODEL_TRANSLATION_VERIFY`** или `vars` в workflow `ydb`. При `Failed to get model` пробуются **`YDBDOC_MODEL_VERIFY_FALLBACKS`** (`yandexgpt/latest`, `yandexgpt-5.1`).
...
- Quality gate + вердикт **ПРИНЯТЬ / НЕ ПРИНИМАТЬ**. По умолчанию «НЕ ПРИНИМАТЬ» **роняет** job и **не создаёт** коммит (ручная правка EN не нужна — перезапуск `doc_translate`). v2: до `YDBDOC_QA_REPAIR_MAX_ROUNDS` (default 2) доп. раундов «исправитель → переводчик» после отказа. Soft merge (коммит при отклонении): `YDBDOC_TRANSLATION_STRICT_MERGE=0`. Если **критик** не отработал — job зелёный, в отчёте «критик недоступен».
