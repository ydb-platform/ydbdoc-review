...
**Отладочная проверка перевода второй моделью** (после генерации — отчёт в **первый комментарий PR с переводом**, в исходном PR только ссылка): в TOML **`[feature].translation_self_check`**, в **`[models].translation_verify`** (slug второй модели; если пусто — `check`). Лимиты — env `YDBDOC_TRANSLATION_SELF_CHECK_*`. Переопределение без пересборки: **`YDBDOC_TRANSLATION_SELF_CHECK`**, **`YDBDOC_MODEL_TRANSLATION_VERIFY`** (приоритет env).

**Строгий перевод:** по умолчанию правки из `git diff` PR накладываются на существующий файл перевода **батчами** (`YDBDOC_TRANSLATE_DIFF_BATCH_CHARS`, по умолчанию 3500); полный перевод файла — только если целевого файла нет или он сильно отстаёт от базы. Откат на «перевести весь файл» при сбое diff выключен (`YDBDOC_TRANSLATE_ALLOW_FULL_FALLBACK=0`); включить — `=1`. Промпты — в каталоге `prompts/` (на русском).
