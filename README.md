...
В defaults без JSON перевод и исправления используют согласованную пользователем модель Yandex Cloud `deepseek-v4-flash` с одной альтернативой `yandexgpt-5-pro`; критик — `yandexgpt-5.1` с альтернативой `yandexgpt-5-lite`. Поддержка отключения reasoning у точного YC deployment не подтверждена: default не передаёт предположительный параметр. Это не подтверждение живой совместимости; см. [управление ответом модели](docs/model-response-controls.md). Секреты: `YANDEX_CLOUD_FOLDER_DOC_REVIEW` и `YANDEX_CLOUD_API_KEY_DOC_REVIEW`. Непустые `YDBDOC_MODEL_TRANSLATE` / `YDBDOC_MODEL_CHECK` переопределяют основные модели; пустые значения сохраняют встроенные настройки. Для других провайдеров и моделей можно передать доверенный `--config` / Action input `config`. Четыре продуктовые переменные остаются обязательными, их workflow передаёт из `${{ vars.* }}` через `env`; Action не обращается к контексту `vars`.
...
`provider` — `eliza` или `yandex_cloud`; для Yandex Cloud дополнительно требуется `folder_id`, base URL указывает OpenAI-compatible `/v1`. `alternative` имеет ту же форму, что `main`. Емкость должна подходить всем заданным endpoint. Вместимость оценивается модельным tokenizer с учётом промпта, глоссария и резерва ответа, сначала для целого файла. UTF-8 байты не считаются токенами. Для YandexGPT `yandexgpt-5-pro`, `yandexgpt-5.1`, `yandexgpt-5-lite` используется официальный YC Tokenizer. Кандидат F05 добавляет для `deepseek-v4-flash` локальный официальный токенизатор закреплённой revision `60d8d70770c6776ff598c94bb586a859a38244f1`, с проверкой SHA-256 и ограниченным форматом system/user. При unset reasoning считается максимум chat/thinking форматов. Точность относительно закреплённого reference не доказывает тождество внутреннего формата YC; живой parity-проверки не было. Независимая приёмка F05 и итоговая F13 ещё не завершены. Подробнее — [токенизация](docs/model-tokenization.md). Неизвестные модели и провайдеры отклоняются до платного обращения. В обычных тестах ответы провайдера явно имитируются. Глоссарий — список пар RU/EN.

Для стандартных моделей заданы синхронные тарифы Yandex AI Studio в рублях за миллион токенов с НДС: DeepSeek V4 Flash — 300 вход / 500 выход / 75 кешированный вход; YandexGPT 5.1 — 800/800, Pro 5 — 1200/1200, Lite 5 — 200/200. Источник: [официальные тарифы YC](https://aistudio.yandex.ru/ru/docs/ai-studio/pricing), проверено 19.09.2026. Стоимость рассчитывается по usage; это не обещание цены документа или подтверждение доступности deployment.
