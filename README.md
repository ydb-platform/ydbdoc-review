...
**Переводчик по умолчанию:** **`yandexgpt-5.1`**. A/B с DeepSeek: **`YDBDOC_MODEL_TRANSLATE=deepseek-v4-flash/latest`** (модель должна быть включена в [Model gallery](https://aistudio.yandex.ru/model-gallery) для folder). При `Failed to get model` пробуются **`YDBDOC_MODEL_TRANSLATE_FALLBACKS`**. Критик (`translation_verify`, Qwen) **не трогайте**. Локально один файл:
