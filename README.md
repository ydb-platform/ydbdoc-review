...
Токен **`GITHUB_TOKEN`** в workflow на стороне базового репозитория **не может** пушить в чужой форк. Нужен **PAT** (сначала можно ваш, позже — у machine user / бота) с правом пуша в ветку head PR при включённой у автора опции **«Allow edits and access to secrets by maintainers»**. Значение кладётся в секрет **`GITHUB_PUSH_TOKEN`** в репозитории, где лежит workflow (например **`ydb-platform/ydb`**), а в `env` action передаётся как `GITHUB_PUSH_TOKEN: ${{ secrets.GITHUB_PUSH_TOKEN }}`. Если секрет **не** создан, в коде используется тот же токен, что и **`GITHUB_TOKEN`** (удобно для PR из ветки в основном репо).

#### Как завести секрет `GITHUB_PUSH_TOKEN` (временно ваш PAT)
...
2. **New repository secret** → имя ровно **`GITHUB_PUSH_TOKEN`** → в поле значения вставьте **PAT** (classic: scope **`repo`** для приватных репо и работы с форками; либо **fine-grained**: доступ к `ydb-platform/ydb`, **Contents** read/write и при необходимости **Pull requests** read).
3. Сохраните. В workflow уже должно быть `GITHUB_PUSH_TOKEN: ${{ secrets.GITHUB_PUSH_TOKEN }}` (см. [`examples/ydb-github-doc-translate-on-label.yml`](examples/ydb-github-doc-translate-on-label.yml)).
4. Для **PR из форка** в условии `if:` уберите проверку `head.repo.fork == false`, иначе job не запустится.
5. Когда появится бот-аккаунт: создайте для него PAT с теми же правами, **обновите** значение секрета `GITHUB_PUSH_TOKEN` и **отзовите** старый личный PAT в [настройках токенов](https://github.com/settings/tokens).
    ...
    steps:
      ...
      - name: Documentation translation review
        ...
        env:
          ...
          GITHUB_PUSH_TOKEN: ${{ secrets.GITHUB_PUSH_TOKEN }}  # пусто → как GITHUB_TOKEN; для форка — секрет PAT
          YDBDOC_REPO_PATH: ${{ github.workspace }}
```

**PR из форка:** задайте секрет **`GITHUB_PUSH_TOKEN`** (см. выше), уберите в `if:` проверку `fork == false`, включите у автора PR **Allow edits by maintainers**. Или отключите job (`if:`), если push в форк не нужен.
...
| `GITHUB_PUSH_TOKEN` | Необязательно: PAT для `git push` в ветку PR (нужен для **форка**; иначе можно не создавать — используется `GITHUB_TOKEN`). Временно — ваш PAT, затем замените на PAT бота. |
