...
- Quality gate + вердикт **ПРИНЯТЬ / ОТКЛОНИТЬ**; по умолчанию «ОТКЛОНИТЬ» **не роняет** job — замечания в отчёте, коммит создаётся (soft merge). Жёсткий режим: `YDBDOC_TRANSLATION_STRICT_MERGE=1`.
