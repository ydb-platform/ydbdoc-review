...
**Проверка и исправление перевода** (в TOML: **`translation_self_check`**, **`translation_repair`**; модель-критик — **`[models].translation_verify`**): после генерации критик сравнивает RU↔EN; при существенных замечаниях переписывает перевод, модель-переводчик подтверждает исправление. Исправленные файлы коммитятся в PR перевода; полный отчёт — **первый комментарий** в этом PR. Отключить только исправление: **`translation_repair = false`** или **`YDBDOC_TRANSLATION_REPAIR=false`**.
...
| `no_commit` | `"true"` → флаг `--no-commit` (только для `mode: run`). |
...
На **PR с переводом** (ветка `ydbdoc-review/pr-<N>` или любой PR, где меняется только EN): повесьте лейбл **`doc_verify`**.
...
- Берёт **все EN-файлы из diff** этого PR.
- **Эталон RU** — содержимое на базовой ветке (`main` / `merge_base_with`).
- **Проверяемый EN** — содержимое на ветке PR.
- Запускает **критик + переводчик** (как при генерации, но **без** нового перевода и без коммита).
- Пишет комментарий с вердиктом **ПРИНЯТЬ / ОТКЛОНИТЬ**; при отклонении job падает (PR не готов к мержу).

Номер исходного doc PR подставляется из заголовка (`Translation of PR 39601`) или флага CLI `--source-pr`.
