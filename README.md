...
4. **Translate:** для каждой пары, где `needs_generation_for ∈ {en, ru}`, переводится **весь файл** (не дельта — потому что качество существующих переводов низкое, и точечная склейка ломается). Файл разбирается на units (`prose`/`table`/`fence`/`diplodoc`), каждый unit — отдельный вызов переводчика по [`prompts/08_translate_segment.txt`](prompts/08_translate_segment.txt), потом сборка + детерминированные CLI-фиксы (yandex-cloud links, `--config-dir`, `ydb sql -s` хвосты).
...
- **Перевод всегда полнофайловый.** Дельта не применяется: качество существующих переводов в YDB-документации низкое, точечная склейка нового перевода с устаревшим EN ломала результат. Стратегия может поменяться, когда документация выровняется по парам.
