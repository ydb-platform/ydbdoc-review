...
- Quality gate + вердикт **ПРИНЯТЬ / ОТКЛОНИТЬ**. По умолчанию «ОТКЛОНИТЬ» **не роняет** job (soft merge). Если **критик** не отработал (`Failed to get model` и т.п.) — job тоже **зелёный**, translation PR создаётся, в отчёте явно написано «критик недоступен». Жёсткий режим: `YDBDOC_TRANSLATION_STRICT_MERGE=1`.
