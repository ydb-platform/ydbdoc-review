...
> Нормативное поведение переводческого конвейера определено только в [`REQUIREMENTS_RU.md`](REQUIREMENTS_RU.md). Описания ниже являются обзором реализации и не расширяют контракт.
...
опциональна (`docker-publish.yml`, workflow_dispatch). Примеры для **ydb** — [`examples/`](examples/).
