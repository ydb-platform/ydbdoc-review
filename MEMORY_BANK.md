...
| When | What |
|------|------|
| 2026-08-28 | **Simple clean rewrite attempt 1 TEST FAIL**: exact candidate `01db731a3f15aaa6be344c7243e39ddefbae6cd6` built and installed cleanly with 24 tests, Ruff and mypy passing, but independent QA found production-blocking defects. Workflow omitted required model configuration; model call IDs collided on the second document; YDB enum serialization and non-atomic transitions broke RESULT/UNKNOWN_BILLED/lease safety; paid calls continued after unknown billing; dependency closure was not published; continue trusted stale lineage instead of live Draft/comment location; terminal comments lacked intent checkpoints; missing labels were accepted; PR45949 E2E used injected fakes rather than production composition/raw pages/blobs; and reports/model metadata were misleading. Real-YDB evidence is separately BLOCKED by absent credentials. No tag, cutover or `doc_translate` is authorized; two independent RCAs precede attempt 2 |
