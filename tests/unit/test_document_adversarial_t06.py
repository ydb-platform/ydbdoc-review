...
import pytest
import yaml
from ydbdoc_review.document import protect, restore, MarkerError, chunk_document, RequestBudget, translation_messages, translate_document, translate_files
...
def run(source, mutation=lambda t:t):
    f = Fake([mutation])
    r = translate_document(source, path='p.md', source_lang='ru', target_lang='en', client=f, choice=CHOICE,
...
def test_marker_attack(mode):
    ...
    t=p.text
    if mode=='drop': t=t.replace(ids[0],'')
    elif mode=='duplicate': t=t.replace(ids[0],ids[0]*2)
    elif mode=='swap': t=t.replace(ids[0],'TEMP').replace(ids[1],ids[0]).replace('TEMP',ids[1])
    elif mode=='unknown': t+='⟦new⟧'
    else: t+='⟦fragment'
    with pytest.raises(MarkerError): restore(p,t)
...
def test_full_request_exact_capacity_and_single_request():
    ...
    f=Fake()
    r=translate_document(s,path='p.md',source_lang='ru',target_lang='en',client=f,choice=CHOICE,budget=b)
    assert len(f.calls)==1 and r.text==s and not r.issues
    assert not RequestBudget(count+9999,10000,COUNT).fits(f.calls[0][0])

@pytest.mark.parametrize('source,injection',[
    ('```python\nx = 1 # Текст\n```\n','Comment\nx = 999 # injected'),
...
@pytest.mark.parametrize('body,replacement',[
    ('title: \'Текст\'\n',"User's guide"),
    ('title: "Текст"\n','The "best" guide'),
    ('config: safe\ntitle: "Текст"\n','Text"\nconfig: hacked\nextra: "'),
    ('config: safe\ntitle: |-\n  Текст\n','Text\nconfig: hacked'),
...
def test_partial_damaged_failure_no_retries():
    ...
    snapshots=[]
    rs=translate_files([('one','First `atom`.'),('two','Second.'),('three','Third.')],source_lang='ru',target_lang='en',client=f,choice=CHOICE,budget=RequestBudget(50000,10000,COUNT),on_progress=snapshots.append)
