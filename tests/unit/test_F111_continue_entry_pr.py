...
def test_F111_source_without_artifact_is_translate_entry() -> None:
    """Service PR no-artifact entry continues translation with Analyze."""
    github = MagicMock()
    github.get_pull.return_value = _pull("feature/docs", service=False)
    ops_ctx = SimpleNamespace(parent_run_id="parent-run", store=None)
    translated = DocJobResult(
        mode="doc_translate",
        pr_number=SOURCE_PR,
    ...
    with (
        ...
        patch(
            "ydbdoc_review.github.workflow._load_continuability_for_continue",
            return_value=_continuability(None),
        ),
        patch(
            "ydbdoc_review.github.workflow.run_doc_translate",
            return_value=translated,
        ...
        result = run_doc_continue(
            ...
            dry_run=True,
            config=_config(),
            instruction="Reopen Analyze and republish when source changed",
        )

    translate.assert_called_once()
    assert translate.call_args.kwargs["pr_number"] == SOURCE_PR
    assert result.mode == "doc_continue"
    assert result.translation_pr_number == translated.translation_pr_number
    verify.assert_not_called()
