...
Для серии независимых дефектов сначала оформите отдельное функциональное
требование и падающие regression-тесты на каждый дефект. Разработчик запускает
только focused-тесты своей задачи. После каждой задачи нужен независимый review.
Полный suite запускается один раз после завершения всей серии, а не после каждого
отдельного теста или исправления. Если задача не стала зелёной за 15 минут,
остановитесь и пересмотрите причину и границы решения.

При восстановлении после сбоя работайте в один поток. Следите за доступной
памятью, числом процессов и прошедшим временем во время работы. Каждый этап
разработки ограничен 15 минутами; зависший этап останавливается для диагностики.
Каждый тест, включая setup и teardown фикстур, ограничен 10 секундами.
Не повышайте этот лимит маркерами: медленный тест должен упасть с traceback для
диагностики. Общая настройка pytest использует signal timeout.
Каждый запуск тестов должен иметь внешний таймаут не более 15 минут с остановкой
дочерних процессов. Перед тестами обновляйте репозиторий (`git pull`), сохраняя
локальные изменения и учитывая параллельные изменения других разработчиков.
