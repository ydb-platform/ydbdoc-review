...
- **R-GL-12** — публикация paid candidate отделена от merge readiness typed-осью `WITHHOLD_INCOMPLETE | WITHHOLD_UNSAFE | PUBLISH_RED | PUBLISH_NORMAL`. Pair error, отсутствующий ожидаемый output, `translate_soft_keep`, source-retaining `ManualAction`, segment alignment failure, deterministic link-contract failure, protect-marker leakage и invalid mandatory navigation YAML всегда запрещают prepare/commit/push/PR. Полный structurally safe candidate с финальным `en_link_target` blocker не превращает path в `completeness_gaps`: blocker хранится на PR-level, candidate публикуется draft/RED, включая impact paths без `PairRunResult`.
- **R-GL-13** — published-blocked candidate обязан иметь native draft state, явный `QA RED, do not merge` banner и blocker summary в translation PR, красную merge recommendation в полном отчёте и `published_red` в source summary, CLI и ops ledger. Exit 0 после успешного создания draft разрешает downstream CI и означает только успешную публикацию artifact; отсутствие translation PR остаётся hard failure. Existing ready-for-review PR перед продолжением переводится обратно в draft.
...
- **R-GL-12/R-GL-13:** top-level `run_doc_translate` tests на real temporary git tree доказывают, что safe final `en_link_target` blocker сохраняет `completeness_gaps == []`, не очищает touched paths, вызывает prepare/commit/push и `create_pull(draft=True)`, а blocker на declaration/redirect impact path без `PairRunResult` остаётся в `final_tree_blockers`. Parameterized matrix доказывает отсутствие prepare/commit/push/PR для incomplete/unsafe сигналов. Existing ready PR получает `convertPullRequestToDraft`; clean candidate остаётся `PUBLISH_NORMAL`.
