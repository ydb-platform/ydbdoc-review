...
файла и историей Memory Bank в вопросах создания PR, порядка QA, повторного
`doc_translate` и обработки блоков кода. Это согласованные требования; соответствие текущего кода ещё
не подтверждено.

### D-001. Перевод, PR и критик

При наличии файлов для перевода (исключение D-009) перевод сохраняется
в ветке и обычном PR до первого критика. Далее:
...
Создание PR и готовность к merge — разные факты. Тип PR определён в D-008.
Поведение при невозможности получить перевод или выполнить GitHub-операцию
этим уточнением отдельно не определено.
...
### D-005. Полнота смысловых блоков

Critic сравнивает authoritative RU и
exact finalized EN целыми предложениями или блоками. Каждый смысловой RU-блок
и каждый сопоставленный EN-блок покрываются ровно один раз в каждой итерации критика. YFM, таблицы и
смешанный Markdown входят в покрытие.
Потерянный блок или недоказанная доставка текста критику дают RED
как незавершённая проверка, а не пустой GREEN. Пакетирование не дробит блок.
Повторяющиеся разделы без якорей сопоставляются по порядку; правильность
их перевода и порядка проверяет модель, а не эвристика уникальности структуры.

### D-006. Критик и автоматические исправления
...
### D-007. Человекочитаемый отчёт

Отчёт начинается с GREEN, YELLOW или RED.
Каждое замечание содержит exact `K`, EN path, реальную строку/диапазон, короткую
поисковую цитату из frozen bytes, простое описание проблемы и ожидаемое
исправление. Внутренние коды вторичны. Недоказанная SHA/location/citation даёт
явный RED integrity item; запрещено выбирать первое похожее совпадение.

### D-002. Новая постановка doc_translate

Каждая новая постановка метки `doc_translate` означает полный перезапуск:

1. Остановить предыдущий запуск для данного исходного PR, если он ещё работает, и дождаться остановки.
2. Удалить текущую ветку перевода данного исходного PR.
3. Создать ветку заново и перевести исходные файлы с нуля.
4. Обеспечить переводной PR и выполнить цикл критика из D-001.
...
Приёмка D-001/D-004: PR существует до critic; critic читает exact `K`; SHA и
байты до/после проверки совпадают; ordinary finding остаётся YELLOW без
автоправки; неполная проверка остаётся RED; отчёт ведёт на реальные строки `K`.
...
Если вторая метка поставлена во время первого запуска, порядок наблюдаемых
действий: остановка первого → подтверждение остановки → удаление ветки →
новый перевод. Первый запуск больше не меняет ветку и вердикт PR.
...
- После сохранения окончательного кандидата и создания PR `doc_translate` в том
  же job запускает цикл критика с максимум двумя автоматическими исправлениями D-001/D-004. Отдельная
  постановка `doc_verify` для этого не требуется.
...
- блоки и фрагменты кода; комментарии внутри блоков переводятся отдельно по D-003;
...
Переводимая проза внутри иначе защищённых конструкций:
...
- человеческие подписи поддерживаемых Mermaid sequenceDiagram и graph/flowchart
  узлов в квадратных скобках, с кавычками или без них, переводятся тем же
  одноразовым проходом; тела обычных `%%` комментариев переводятся отдельными
  запросами по D-003.
...
Неподдерживаемая грамматика остаётся защищённой; остаточная кириллица внутри
Mermaid code block даёт warning с файлом и строкой по D-003, а не RED.

Любые другие ключи front matter не переводятся и сохраняются байт-в-байт в пределах защищённых зон.

Исключение D-003: кириллица в блоках кода допустима и даёт только warning с файлом и строкой. Вне этого исключения защита исходного литерала не разрешает остаточную кириллицу в финальном EN:
...
- После полной сборки каждого кандидата critic проверяет целые смысловые RU/EN-блоки.
- При замечаниях выполняется автоматическое исправление и новая проверка:
  максимум два исправления и три проверки; без замечаний — раннее завершение.
- Технические retries ограничены отдельно; валидное замечание запускает
  исправление в пределах лимита, а не завершает весь цикл.
...
- остаточная кириллица во всём финальном EN-тексте, включая защищённые зоны,
  anchors, HTML, include, inline code и Markdown/YAML front matter;
  для блоков кода действует D-003: кириллица даёт warning, а не красную проверку;
...
без исключений парсера после всех restores и поздних ремонтов, до фиксации `K`;
для распознанных блоков кода применяется warning-исключение D-003.
...
- **R-GL-13 — пересмотрено D-001/D-004–D-007:** переводной PR создаётся до
  critic. Каждый проверяемый кандидат неизменен во время проверки; между
  проверками конвейер сохраняет исправления в ветке согласно D-001/D-004.
  Итоговый вердикт относится к последнему проверенному SHA. RED означает
...
- Замечания к полученному переводу не отменяют созданные branch/commit/PR;
  critic не исправляет `K`, а публикует понятный отчёт (D-001, D-004–D-007).
...
Нормальный исход «Нет файлов для перевода» (D-009) не требует переводного PR.
...
- цикл через production entrypoint выполняет максимум три проверки и два исправления; проверяются раннее завершение, новые SHA после исправлений, отсутствие третьего исправления и итоговый отчёт на последнем SHA;
...
- **R-GL-12/R-GL-13:** действующая приёмка следует D-001/D-004–D-007: PR
  создаётся до critic и остаётся доступным при ошибках; critic читает exact `K`,
  не исправляет его и публикует один итоговый отчёт. Формулировка про два
  исправления и три проверки является отменённой историей. Приёмка полного
  перезапуска остаётся в D-002.
...
## Приёмка решений D-001–D-009 после восстановления работы

Реализация и соответствующие regression-тесты перечислены в
[банке знаний](docs/memory-bank/10-product-decisions.md#реализация-d-001d-009-восстановление-после-сбоя).
Тесты production orchestration должны подтверждать создание PR до critic,
отдельный SHA после каждого из максимум двух исправлений и отсутствие
изменений после третьего critic. Новая метка не допускает Analyze no-op;
остановка предыдущего Actions run подтверждается до удаления ветки.
Перед проверками разработчик синхронизирует текущий main, сохраняя параллельные изменения.
