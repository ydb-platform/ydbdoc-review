...
- **R-GL-9** — разрешимость baseline href не доказывает семантическую эквивалентность section target. `prefer_baseline_href_when_fragment_missing` не заменяет source-owned ASCII fragment другим baseline fragment; исключение только то же доказанное implicit heading auto-slug соответствие из R-GL-8. После записи всех dependency overlays поздний repair повторно проверяет source contract, а missing stable fragment ведёт к owner translation/declaration либо blocker, но не к retarget на соседнюю существующую секцию. Exact-ASCII диагностика при равном общем числе link-slots использует document-order pairing; при неравном числе path-local pairing допустим только для единственной unmatched пары.
- **R-GL-10** — pre-existing stable-fragment href на source-diff странице ставит RU owner в translation scope, только если RU target однозначно объявляет fragment, а EN target его не объявляет. Broken RU href без RU declaration не расширяет scope. Для такого ambient broken RU path разрешено сохранить tip EN path только с тем же decoded fragment и только когда полный EN `path#fragment` разрешяется; подмена fragment запрещена. Это уточняет R-GL-4a и §6.233, не отменяя их защиту от произвольного ambient scope.
- **R-GL-11** — post-translate обработка обязана использовать две временные проекции: финальный EN candidate читается поверх `merge_base_with`, а RU-владелец exact fragment читается из `ru_content_ref` исходного PR. Правило сохранения разрешимого tip EN `path#fragment` с тем же ASCII fragment применяется и к обычному результату LLM, не только к deterministic mirror fast-path. Приёмочный тест проходит полный локальный маршрут `load contents → run_pr_translation → apply → declare → en_link_target` на production-shaped #40385 fixture и одновременно проверяет `security-auth` и `certificate-auth-config`; helper-only тесты недостаточны.
...
- **R-GL-9:** fragment-repair fixture сохраняет `#security-auth`/`#certificate-auth-config`/`#tls`, даже если baseline указывает на существующий другой explicit anchor; доказанный implicit heading slug по-прежнему может использовать локализованный baseline. Production-подобный `authentication.md` с двумя RU href на `auth_config.md`, одним EN href на тот же path и равным общим числом link-slots сообщает `#certificate-auth-config`→`#iam-auth-config`, а не ошибочную пару с `#security-auth`.
- **R-GL-10:** #40385 scope fixture с pre-existing `auth_config#security-auth` (не объявлен в RU owner) и `auth_config#certificate-auth-config` (explicit в RU, отсутствует EN) ставит `auth_config.md` в `doc_from_main` из-за certificate fragment, не из-за broken security fragment. `prefer_resolvable_en_hrefs` сохраняет `security_config.md#security-auth` вместо broken `auth_config.md#security-auth`, но не допускает различающиеся ASCII fragments.
- **R-GL-11:** production-shaped #40385 fixture проходит полный post-translate lifecycle локально: normal LLM path сохраняет tip `security_config.md#security-auth`; declaration читает `auth_config.md` из source `ru_content_ref`, добавляет `{#certificate-auth-config}` в EN candidate; финальный `apply_en_link_target_checks` возвращает `[]` без запуска GitHub Actions.
