...
2. До critic закончить сборку полных EN-файлов и все механические изменения
   структуры, ссылок, навигации и форматирования. Зафиксировать точный commit
   SHA кандидата `K` в ветке перевода.
3. Critic один раз проверяет смысл авторитетного RU против точного EN из `K`.
   Он не меняет перевод и не запускает переводчик повторно.
4. Записать итоговый вердикт и оставшиеся замечания в PR. Обычная неточность
   перевода даёт понятное YELLOW-предупреждение, с которым техпис работает сам.
5. GREEN допустим только после полной и доказанной проверки всех смысловых
   блоков `K` без открытых замечаний.
...
Порядок строго такой: перевод всех кусков
прозы, сборка всех EN-файлов, все механические repairs, фиксация `K`, затем
critic. Critic, отчёт и публикация относятся к одному `K`. После начала critic
content mutations запрещены; любое изменение создаёт новый кандидат и требует
новой проверки. Замечание critic не запускает переводчик. Существующий fallback
переводчика при непригодном ответе или исчерпанной транспортной ошибке сохраняется.
...
и каждый сопоставленный EN-блок покрываются ровно один раз. YFM, таблицы и
...
### D-006. Read-only critic

На корректный пакет приходится один смысловой
запрос. Обычная неточность нормализуется в YELLOW с объяснением и необязательным
советом; предложенный текст не применяется. Отказ, пустой/невалидный ответ или
техническая ошибка critic после ограниченного recovery дают RED. Запрещены
специальные алгоритмы для `and`/`or`, отрицаний, пунктуации и конкретных фраз.
...
- После сохранения окончательного кандидата и создания PR `doc_translate` в том
  же job запускает одну read-only смысловую проверку D-001/D-004. Отдельная
...
- После полной сборки и фиксации `K` critic один раз проверяет целые смысловые
  RU/EN-блоки согласно D-004–D-006.
- Обычное замечание не запускает локальное исправление, повторный перевод или
  второй critic pass; оно публикуется как понятный YELLOW для техписа.
- Технические retries и bounded recovery разрешены только пока critic не дал
  валидный semantic response. После валидного ответа цикл заканчивается.
...
- read-only critic не применяет локальные исправления и не запускает повторный перевод;
