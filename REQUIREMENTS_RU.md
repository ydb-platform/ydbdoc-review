...
Отдельный диагностический отчёт о более позднем RU drift рассматривает только
точные пути исходного PR и уже принятые планировщиком зависимости
`doc_ru_paths | nav_ru_paths` за вычетом исходных путей. Оба набора фиксируются
один раз до обхода first-parent коммитов. Совпадение проверяется по точной строке
Git path без нормализации слешей и без рекурсивного поиска новых зависимостей.
Коммиты без разрешённых путей отбрасываются до чтения subject и обращения к API
pull request. Этот отчёт не расширяет translation scope и не меняет quality
verdict.
...
- Заголовок `Confirmed merge association` означает только, что GitHub API вернул
  `merged=true`, тот же номер PR и точный `merge_commit_sha` рассматриваемого
  коммита. Это подтверждение происхождения merge, а не проверка качества PR,
...
- **R-GL-18:** exact offline #51079 replay фиксирует H0 `0aa50f3`, H `673b924`,
  B `7886ff8` и исходный дефектный K `a1be7c4` полными SHA и SHA-256 каждого
  snapshot. Реальный локальный pipeline переводит ровно шесть Markdown и один
  TOC из исходного diff двух RU-файлов, сохраняет glossary coverage и Mermaid
  skeletons; неизвестные reads/model segments блокируются. VERIFY читает полный
  локальный SHA через Git, отчёт показывает его штатное 12-символьное имя и
  проверяет оба поля результата против точных байтов. Исходный K и семь отдельных
  мутаций дают non-green при отключённой сети. Fixture не заменяет восьмифайловый
  R-GL-16. Production retry, проверка содержания и зелёные `doc_verify`/build
  на одном production SHA остаются отдельной приёмкой.
