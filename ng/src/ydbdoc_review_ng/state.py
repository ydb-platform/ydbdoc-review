...
class YdbState(StatePort):
    ...
    def record_accepted_continue(self, lineage_id: str, accepted: AcceptedContinue) -> MutationResult:
        ...
        def op(tx):
            ...
            tx.execute(f"DECLARE $repo AS Utf8; DECLARE $id AS Utf8; DECLARE $count AS Uint32; DECLARE $decisions AS Json; $now=CurrentUtcTimestamp(); UPDATE `{table}` SET typed_decisions=$decisions,continue_count=$count,updated_at=$now,expires_at=Unwrap($now+Interval('P14D'),'lineage expiry') WHERE repository=$repo AND lineage_id=$id;", params)
