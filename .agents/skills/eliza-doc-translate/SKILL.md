...
7. **Green QA is necessary, not sufficient.** Before reporting success, perform
   an independent review of the exact translation PR checkout. Compare the
   source PR scope and RU/EN twins; verify moves/deletions, TOC, redirects,
   internal paths/fragments, technical atoms, structure, and meaning. Do not
   reuse the critic verdict as independent evidence. Fix every confirmed issue,
   re-run QA on the new SHA, and repeat the independent review.
...
- [ ] 6. On 🟢: independently audit exact PR bytes and source-PR scope
- [ ] 7. Fix independent findings, re-run QA, and re-audit
- [ ] 8. Stop only when QA is 🟢 and independent audit is clean
```
...
3. Latest **ydbdoc-review** report comment:
   - 🟢 → proceed to the mandatory independent audit below; not done yet.
...
4. Independently inspect the exact reported checkout:
   - compare the source PR changed-file set with the translation PR and reject
     unrelated additions unless the user explicitly approved them;
   - for every RU add/delete/move, confirm the matching EN operation, including
     removal of obsolete EN files;
   - compare RU and EN article structure and meaning manually, not through the
     same critic model;
   - resolve every changed internal link and fragment against the EN tree;
   - inspect RU/EN TOC symmetry and locale-specific redirects;
   - run `git diff --check` and confirm the report checkout SHA equals PR HEAD.

If this audit finds a defect after green QA, treat it as a pipeline regression:
add a test using that PR as fixture data, fix the logic, publish the fix, rerun
translation/QA, and repeat this audit.
