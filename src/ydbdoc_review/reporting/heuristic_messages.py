    ...
    if message.startswith("orphan_toc_page:"):
        return HeuristicReviewerDetail(
            ...
            suggestion=(
                ...
                "В какой раздел целевого TOC поместить страницу? Укажите адрес через "
                "`/ydbdoc continue <адрес>`, место как `toc=<путь>`, и добавьте redirect; "  # noqa: RUF001
                "до этого старый маршрут удалять нельзя."
            ),
        )
    if message.startswith("outbound_fragment:"):
        return HeuristicReviewerDetail(
            problem=problem,
            suggestion=(
                "Фрагмент не удалось однозначно перенести. Укажите назначение "
                "через `/ydbdoc continue <адрес#fragment>`; до этого старый "
                "`path#fragment` удалять нельзя."
            ),
        )
...
def _humanize_heuristic_problem(message: str) -> str:
    ...
    if message.startswith("outbound_fragment:"):
        detail = message.split(":", 1)[1].strip()
        return (
            "В EN битая внутренняя ссылка или якорь "
            f"(детерминированная проверка): {detail}"
        )
    if message.startswith("en_link_target:"):
        detail = message.split(":", 1)[1].strip()
        return (
            "В EN битая внутренняя ссылка или якорь "
            f"(детерминированная проверка): {detail}"
