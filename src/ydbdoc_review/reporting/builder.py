...
def _is_unresolved_dependency_warning(warning: str) -> bool:
    """Recognize dependency-closure warnings that must remain visible as RED."""
    return any(
        marker in warning
        for marker in (
            "link dependency budget exhausted",
            "не удалось восстановить доступный бюджет",
            "dependency excluded",
            "RU source is unavailable",
        )
    )
...
def _merge_recommendation(result: PRTranslationResult) -> tuple[str, str]:
    ...
    if any(_is_unresolved_dependency_warning(w) for w in result.yellow_warnings):
        return "🔴", "не мержить — опубликован RED: есть нерешённые зависимости"
...
def build_full_report(
    ...
    *,
    ...
    dependency_warnings = [
        ...
        if _is_unresolved_dependency_warning(warning)
    ]
    ordinary_warnings = [
        warning
        for warning in result.yellow_warnings
        if not _is_unresolved_dependency_warning(warning)
    ]
    dependency_section = ""
    if dependency_warnings:
        dependency_section = (
            "## QA RED, do not merge: нерешённые зависимости\n\n"
            "Готовый перевод опубликован, исходная ссылка сохранена. "
            "Для каждого пути нужен отдельный перевод или разрешённый "
            "`doc_continue`; расширение задания остановлено.\n\n"
        )
        for i, warning in enumerate(dependency_warnings, start=1):
            dependency_section += f"{i}. {warning}\n\n"
    ...
    if ordinary_warnings:
        ...
        for i, warning in enumerate(ordinary_warnings, start=1):
    ...
    if not file_runs and not nav_runs:
        ...
        if errors or nav_errors:
            ...
            return body
        if completeness_section or final_tree_section or dependency_section or yellow_section:
