...
class _ExtractState:
    ...
    def walk_block(
        ...
        if isinstance(block, Paragraph):
            ...
            # Optional quoted title is prose (REQUIREMENTS_RU.md §5); markers
            # stay structural and are never segmented.
            if block.title is not None and block.title.strip():
                self.segments.append(
                    Segment(
                        id=self.next_id(),
                        kind=SegmentKind.NOTE_TITLE,
                        path=path + [f"note:{block.note_type}"],
                        text=block.title,
                        placeholders=[],
